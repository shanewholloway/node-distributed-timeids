...
ava('smoke', t => t.truthy(timeids))

ava.cb('example', t => {
  const mock = {log: obj => obj && t.pass() }
  t.plan(4 * 2)
  require('../example.js')
    .main(mock, t.end)
})
...
ava('full', t => {
  ...
  return new Promise(rs=>setTimeout(rs, 300))
    ...
    .then(()=> {
      ...
      t.true(isOrdered(outer.map(r=>r[0])))
      // last element of each sub list should be ordered
      t.true(isOrdered(outer.map(r=>r[r.length-1])))
    })
